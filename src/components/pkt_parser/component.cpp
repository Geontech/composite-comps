...
auto pkt_parser::process() -> composite::retval {
    ...
    auto do_send = true;
    meta.annotations["contiguous"] = "true";
    ...
    if (m_transport == transport::sdds) {
        ...
        if (packet.pp_id() && ((seq_num % 32) != 31)) [[unlikely]] {
            logger()->warn("invalid SDDS packet received, pp_id=true, seq_num={}", seq_num);
        } else if (!packet.pp_id() && ((seq_num % 32) == 31)) [[unlikely]] {
            logger()->warn("invalid SDDS packet received pp_id=false, seq_num={}", seq_num);
        }
