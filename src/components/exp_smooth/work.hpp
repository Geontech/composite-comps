...
private:
    // Helper: scalar processing for any range
    static auto process_scalar_range(float* curr, float* prev, std::size_t start, std::size_t end, float alpha) -> void {
        ...
        for (auto i = start; i < end; ++i) {
            auto sanitized = !std::isfinite(curr[i]) ? prev[i] : curr[i];
            curr[i] = (alpha * sanitized) + (one_minus_alpha * prev[i]);
        }
    }
    ...
    static auto process_avx2_aligned(float* curr, float* prev, std::size_t size, float alpha) -> void {
        ...
        for (; i + STRIDE_256 <= size; i += STRIDE_256) {
            ...
            auto bad_mask = _mm256_or_ps(nan_mask, inf_mask);

            auto sanitized = _mm256_blendv_ps(curr_data, prev_data, bad_mask);
            sanitized = _mm256_mul_ps(sanitized, alpha_vec);
            auto result = _mm256_fmadd_ps(prev_data, one_minus_alpha_vec, sanitized);
    ...
    static auto process_avx512_aligned(float* curr, float* prev, std::size_t size, float alpha) -> void {
        ...
        for (; i + STRIDE_512 <= size; i += STRIDE_512) {
            ...
            auto bad_mask = nan_mask | inf_mask;

            auto sanitized = _mm512_mask_blend_ps(bad_mask, curr_data, prev_data);
            sanitized = _mm512_mul_ps(sanitized, alpha_vec);
            auto result = _mm512_fmadd_ps(prev_data, one_minus_alpha_vec, sanitized);
...
private:
    ...
    static auto process_avx2_aligned(double* curr, double* prev, std::size_t size, double alpha) -> void {
        ...
        for (; i + STRIDE_256 <= size; i += STRIDE_256) {
            ...
            auto bad_mask = _mm256_or_pd(nan_mask, inf_mask);

            auto sanitized = _mm256_blendv_pd(curr_data, prev_data, bad_mask);
            sanitized = _mm256_mul_pd(sanitized, alpha_vec);
            auto result = _mm256_fmadd_pd(prev_data, one_minus_alpha_vec, sanitized);
    ...
    static auto process_avx512_aligned(double* curr, double* prev, std::size_t size, double alpha) -> void {
        ...
        for (; i + STRIDE_512 <= size; i += STRIDE_512) {
            ...
            auto sanitized = _mm512_mask_blend_pd(bad_mask, curr_data, prev_data);
            sanitized = _mm512_mul_pd(sanitized, alpha_vec);
            auto result = _mm512_fmadd_pd(prev_data, one_minus_alpha_vec, sanitized);
