    ...
    m_stat_thread = std::jthread([this](std::stop_token token) {
        while (!token.stop_requested()) {
            ...
            if (!token.stop_requested()) {
                ...
                logger()->trace(
                    "statistics: pkts_recvd_user={}, pkts_processed={} pkts_recvd_kernel={} pkts_dropped_kernel={}",
                    stats.pkts_recvd_user, stats.pkts_processed, stats.pkts_recvd_kernel, stats.pkts_dropped_kernel
                );
            }
        }
    });
