...
auto dpdk_udp::receive(std::stop_token token) -> void {
    ...
    auto bdf = net::find_vfio_net_bdf();
    if (!bdf){
        throw std::runtime_error("No VFIO network device found");
    }
    int dev_numa = net::pci_numa_node(*bdf);
    // m_logger->trace("Using PCI device: {}", bdf);
